...
@respx.mock
def test_create_document_dual_writes_meta(kb_client: TestClient) -> None:
    ...
    with session_factory() as session:
        row = session.get(KbDocumentMeta, ("doc-demo", "role-sales"))
...
def test_get_document_reads_meta_and_agent_chunks(kb_client: TestClient) -> None:
    ...
    response = kb_client.get(
        "/api/admin/kb/documents/doc-detail",
...
@respx.mock
def test_delete_document_removes_meta_and_calls_agent(kb_client: TestClient) -> None:
    settings = get_settings()
    respx.post(f"{settings.AGENT_URL}/internal/kb/ingest").mock(
        return_value=httpx.Response(
            200,
    ...
    response = kb_client.delete(
        "/api/admin/kb/documents/doc-del",
        params={"role_id": "role-sales"},
    )
    ...
    session_factory = get_session_factory(engine)
    with session_factory() as session:
        assert session.get(KbDocumentMeta, ("doc-del", "role-sales")) is None
    engine.dispose()


@respx.mock
def test_patch_document_reingests_and_updates_meta(kb_client: TestClient) -> None:
    settings = get_settings()
    respx.post(f"{settings.AGENT_URL}/internal/kb/ingest").mock(
        side_effect=[
            httpx.Response(
...
def test_create_document_does_not_write_meta_when_ingest_fails(kb_client: TestClient) -> None:
    ...
    session_factory = get_session_factory(engine)
    with session_factory() as session:
        assert session.get(KbDocumentMeta, ("doc-fail", "role-sales")) is None
    engine.dispose()
