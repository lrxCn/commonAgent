...
- `load_memory` 节点在读取 checkpoint、summary、mem0 后，同时生成兼容 `turn_type` 和结构化 `IntentDecision`。
- 当前 graph 热路径使用确定性 `classify_intent()`：normalize -> signals -> rules，不调用 LLM。
- `turn_type` 仍是兼容路由字段；`IntentDecision.route` 是控制面结构化结果，二者分歧会记录 `intent_conflict` 和 fallback。
...
- 纯入口：[engine.py](/Users/liurixing/Documents/codes/ai/commonAgent/agent/src/intent/engine.py:1)
...
- intent conflict / low confidence
- policy denied
- memory missing
- RAG empty / weak hit
...
- Graph 接入：[test_intent_shadow_graph.py](/Users/liurixing/Documents/codes/ai/commonAgent/agent/tests/test_intent_shadow_graph.py:1)
