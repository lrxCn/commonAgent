...
- `post_turn_jobs`：异步调度 summary 和用户记忆写入；有 `memory_write_record` 时走 `store_structured_record`（Store profile），否则走 `extract_and_store`（langmem inferred）；`memory_query` 路径跳过 memory write。
