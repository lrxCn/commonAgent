...
def test_valid_chat_request_parses() -> None:
    ...
    assert req.context.role_id == "role-sales"
...
def test_context_tools_default_empty() -> None:
    ctx = RequestContext.model_validate({"user_id": "u1", "role_id": "r1"})
    assert ctx.tools == []


def test_missing_context_role_id_fails() -> None:
    payload = {
        **_VALID_CHAT_PAYLOAD,
    ...
    assert any(e["loc"] == ("context", "role_id") for e in errors)
